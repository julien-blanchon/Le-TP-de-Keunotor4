...
let%test _ = padovan 0 = 0 (* padovan(0) = 0 --> 0 addition *)
let%test _ = padovan 1 = 0 (* padovan(1) = 1 --> 0 addition *)
let%test _ = padovan 2 = 1 (* padovan(2) = 1 --> 0 addition *)
let%test _ = padovan 3 = 0 (* padovan(3) = padovan(1) + padovan(0) --> 1 addition*)
let%test _ = padovan 4 = 1 (* padovan(4) = padovan(2) + padovan(1) --> 1 addition*)
let%test _ = padovan 5 = 1 (* padovan(5) = padovan(3) + padovan(2) --> 1 addition *) (*--> 2 additions*)
                                        (* padovan(3) = padovan(1) + padovan(0) --> 1 addition*)      
let%test _ = padovan 6 = 1 (* padovan(6) = padovan(4)->1 + padovan(3)->1 *) (*--> 2 additions*)
let%test _ = padovan 7 = 2 (* padovan(7) = padovan(5)->2 + padovan(4)->1 *) (*--> 3 additions*)
let%test _ = padovan 8 = 2 (* padovan(8) = padovan(6)->2 + padovan(5)->2 *) (*--> 4 additions*)
let%test _ = padovan 9 = 3 (* padovan(9) = padovan(7)->3 + padovan(6)->2 *) (*--> 5 additions*)
let%test _ = padovan 10 = 4(* padovan(10) = padovan(8)->4 + padovan(7)->3 *) (*--> 7 additions*) 
(* On aurait pu utilisé bcp moins d'additions  *)

(* Complexité de C(n) = C(n-3) + C(n-2) <-> C(n) = 2C(n-2) <-> C(n) = 2^{n/2} *)
...
let dragon (xa,ya) (xb,yb) n = 
   if n=0
      ...
      dessine_segment ((xa +. xb)/.2. +. (yb-.ya)/.2., (ya +. yb)/.2. +. (xa-.yb)/.2.) (xb,yb);
      dessine_segment (xa,ya) ((xa +. xb)/.2. +. (yb-.ya)/.2., (ya +. yb)/.2. +. (xa-.yb)/.2.)
