...
(******************************************************************************)
(*   fonction de retrait d'un éléments dans un arbre                          *)
(*   signature  : retrait : 'a -> 'a arbre -> 'a arbre                        *)
(*   paramètres : - un éléments (caractères dans le cas d'un dico)            *)
(*                - un arbre n-aire                                           *)
(*   résultat   : l'arbre n-aire avec l'élément supprimer                     *)
(******************************************************************************)
let rec retrait_arbre lc (Noeud (b, lb)) =
  ...
  | c::qlc ->
     let r = recherche c lb in
       match r with
       | None -> Noeud(b, lb) (*Pas de resultat de recherche on renvois l'arbre*)
       | Some a -> Noeud(b, maj c (retrait_arbre qlc a) lb)


let rec parcours Noeud(b, lb) = 
   let liste = List.flatten(List.map parcours_aux lb) in 

   if b=true then []::liste
   else liste 

   and parcours_aux (c,a) = List.map(fun m -> c::m)(parcours a)
